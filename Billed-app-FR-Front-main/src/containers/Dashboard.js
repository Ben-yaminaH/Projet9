...
// *** update 
  ...
  handleEditTicket(e, bill, bills) {
    // Deselect all tickets from all lists
    $('.bill-card').css({ background: '#0D5AE5' });
  
    // Toggle selection state for the clicked ticket
    ...
    // Attach handlers for modal actions
...
// ** update
  ...
  handleShowTickets(e, bills, index) {
    // Initialize unique counters for each list
    ...
    // Increment the counter for this specific index
    this.counters[index]++;
  
    // Attach click handlers for tickets in the displayed list
    filteredBills(bills, getStatus(index)).forEach(bill => {
      $(`#open-bill${bill.id}`).off("click"); // Remove previous listeners
